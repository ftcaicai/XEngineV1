				  ...
				  const SoldierData& soldierData,const std::vector<CBuilderData>& vecBuilderData,const std::map<int,CampData>& mapCampData)
{
	...
	CaculateResourceMineStore();	//重新计算金矿和油矿中的储量之后 需要重新存数据库 
				// 虽说此处并不具有必要性 因为玩家下线的时候 总之是可以存的了 但是如此做法会使得逻辑清晰
	//CMysql::Instance()->SavePlayer(m_iAccount);	// 由于此时还没有插入map 所以等mgr中插入完毕之后 再行save Player
...
void CPlayer::InitBuildFactor()
{
	...
	for (;it!=m_oBuildings.end();it++)
	{
		if (it->m_nID == 1)
		{
			InitTownLevel(it->m_nLevel);
		}

		if(it->m_nID == 6)
		{
			// handle camp
			CCamp *pCamp = new CCamp(it->m_nUniID,it->m_nLevel,this);
			m_mapUniID2Camp[it->m_nUniID] = pCamp;

		}

		if(it->m_nID == 7)	// maybe later change id to type to calculate population
		{
			_AddPopulation(it->m_nLevel);
		}
		...
		if(it->m_nID == 8)
		{
			...
			if(m_lab == NULL)
			{
				m_lab = new CLab(this,it->m_nLevel);
			}
		...
		if(it->m_nID == 11)
		{
			CBuilder *pBuilder = new CBuilder(it->m_nUniID,this);
			m_mapBuilder[it->m_nUniID] = pBuilder;
...
void CPlayer::InitSoldierListData()
{
