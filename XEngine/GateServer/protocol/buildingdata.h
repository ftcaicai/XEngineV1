...
class CPopStationData : public CData
{
public:
	CPopStationData()
	{ 
	...
	CPopStationData(const CPopStationData& rhs)
		:m_iUniID(rhs.m_iUniID)
		,m_iCurPopulation(rhs.m_iCurPopulation)
		,m_mapID2Count(rhs.m_mapID2Count)
	{ 
	}

	virtual ~CPopStationData()
	{
	}

	CPopStationData& operator = (const CPopStationData& rhs)
	{
		if(&rhs != this)
		{
			m_iUniID			= rhs.m_iUniID;
...
public:
	int			m_iUniID;	
	int			m_iCurPopulation;
	std::map<int,int> m_mapID2Count;
};
