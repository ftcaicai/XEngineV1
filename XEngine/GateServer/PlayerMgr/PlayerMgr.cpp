						 ...
						 const std::map<int,CPopStationData>& mapPopStationData,const std::vector<CPlayerWarReportData> &vPlayerWarReport,__time64_t ProtectTime)
{
	...
	CPlayer * pPlayer  = new CPlayer(dwConnID,iAccountID,strName,iRMB,iGold,iOil,rVecBuilding,rVecStoreData,storetime,labdata,soldierdata,vecBuilderData,mapCampData,mapPopStationData,vPlayerWarReport,ProtectTime);
	if(pPlayer == NULL)
	{
		printf("Create Player Error ",dwConnID,iAccountID,strName.c_str(),iRMB);
		return false;
	}
